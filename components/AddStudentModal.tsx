      ...
      <div className="bg-white text-slate-800 rounded-3xl p-8 max-w-4xl w-full shadow-2xl relative max-h-[95vh] overflow-y-auto border-4 border-pink-100 custom-scrollbar">
        ...
        <form onSubmit={handleAddStudentSubmit} className="mt-6 space-y-6 text-xs">
          ...
          <div className="border border-emerald-200 rounded-2xl overflow-hidden shadow-sm">
            <div className="bg-[#1b5e20] text-white font-extrabold px-4 py-2 text-sm uppercase tracking-wider flex items-center justify-between">
              <span>5. Payment Details & Initial Fee Collection</span>
              <button
                type="button"
                onClick={() => {
                  const currentItems = newStudentForm.payment_items || [
                    { id: '1', fee_head: 'Registration Fee', month: '', custom_head_name: '', amount: 1000, discount: 0 },
                    { id: '2', fee_head: 'Monthly Fee', month: 'January 2027', custom_head_name: '', amount: 3500, discount: 0 }
                  ];
                  const updated = [
                    ...currentItems,
                    { id: String(Date.now() + Math.random()), fee_head: 'Other', month: '', custom_head_name: '', amount: 500, discount: 0 }
                  ];
                  setNewStudentForm({
                    ...newStudentForm,
                    payment_items: updated,
                    total_fee: updated.reduce((sum: number, it: any) => sum + Math.max(0, Number(it.amount || 0) - Number(it.discount || 0)), 0)
                  });
                }}
                className="px-2.5 py-1 bg-white text-emerald-800 font-black rounded-lg hover:bg-emerald-50 transition text-[10px] uppercase flex items-center gap-1 cursor-pointer"
              >
                ➕ Add Item
              </button>
            </div>
            <div className="p-4 bg-slate-50/50 space-y-4">
              ...
              <div className="space-y-3">
                {(newStudentForm.payment_items || [
                  { id: '1', fee_head: 'Registration Fee', month: '', custom_head_name: '', amount: 1000, discount: 0 },
                  { id: '2', fee_head: 'Monthly Fee', month: 'January 2027', custom_head_name: '', amount: 3500, discount: 0 }
                ]).map((item: any, idx: number) => (
                  <div key={item.id} className="p-3 bg-white border border-slate-200 rounded-xl grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
                    <div>
                      <label className="block font-bold text-slate-600 text-[10px] mb-1">Fee Head</label>
                      <select
                        value={item.fee_head}
                        onChange={(e) => {
                          const currentItems = [...(newStudentForm.payment_items || [
                          ...
                          const idxToUpdate = currentItems.findIndex(it => it.id === item.id);
                          if (idxToUpdate !== -1) {
                            currentItems[idxToUpdate].fee_head = e.target.value;
                            if (e.target.value === 'Registration Fee') {
                              currentItems[idxToUpdate].amount = 1000;
                              currentItems[idxToUpdate].month = '';
                            } else if (e.target.value === 'Monthly Fee') {
                              currentItems[idxToUpdate].amount = 3500;
                              currentItems[idxToUpdate].month = 'January 2027';
                            } else {
                              const matched = feeHeads.find(h => h.name === e.target.value);
                              currentItems[idxToUpdate].amount = matched ? Number(matched.default_amount) : 500;
                              currentItems[idxToUpdate].month = '';
                            }
                          }
                          setNewStudentForm({
                            ...newStudentForm,
                            payment_items: currentItems,
                            total_fee: currentItems.reduce((sum: number, it: any) => sum + Math.max(0, Number(it.amount || 0) - Number(it.discount || 0)), 0)
                          });
                        }}
                        className="w-full bg-white border border-slate-300 rounded-xl px-2.5 py-1.5 outline-none text-xs font-semibold"
                      >
                        <option value="Registration Fee">Registration Fee</option>
                        <option value="Monthly Fee">Monthly Fee</option>
                        {feeHeads.filter(h => !h.is_system).map((h: any) => (
                    ...
                    {item.fee_head === 'Monthly Fee' && (
                      <div>
                        <label className="block font-bold text-slate-600 text-[10px] mb-1">Select Month</label>
                        <select
                          value={item.month || 'January 2027'}
                          onChange={(e) => {
                            const currentItems = [...(newStudentForm.payment_items || [])];
                            const idxToUpdate = currentItems.findIndex(it => it.id === item.id);
                            if (idxToUpdate !== -1) currentItems[idxToUpdate].month = e.target.value;
                            setNewStudentForm({ ...newStudentForm, payment_items: currentItems });
                          }}
                          className="w-full bg-white border border-slate-300 rounded-xl px-2.5 py-1.5 outline-none text-xs font-semibold"
                        >
                          {MONTHS_LIST.map((m: string) => (
                    ...
                    {item.fee_head === 'Other' && (
                      <div>
                        <label className="block font-bold text-slate-600 text-[10px] mb-1">Fee Name</label>
                        <input
                          type="text"
                          required
                          ...
                          onChange={(e) => {
                            const currentItems = [...(newStudentForm.payment_items || [])];
                            const idxToUpdate = currentItems.findIndex(it => it.id === item.id);
                            if (idxToUpdate !== -1) currentItems[idxToUpdate].custom_head_name = e.target.value;
                            setNewStudentForm({ ...newStudentForm, payment_items: currentItems });
                          }}
                          className="w-full bg-white border border-slate-300 rounded-xl px-2.5 py-1.5 outline-none text-xs font-semibold"
                        />
                    ...
                    <div>
                      <label className="block font-bold text-slate-600 text-[10px] mb-1">Amount (₹)</label>
                      <input
                        type="number"
                        required
                        value={item.amount}
                        onChange={(e) => {
                          const currentItems = [...(newStudentForm.payment_items || [])];
                          const idxToUpdate = currentItems.findIndex(it => it.id === item.id);
                          if (idxToUpdate !== -1) currentItems[idxToUpdate].amount = parseFloat(e.target.value) || 0;
                          setNewStudentForm({
                            ...newStudentForm,
                            payment_items: currentItems,
                            total_fee: currentItems.reduce((sum: number, it: any) => sum + Math.max(0, Number(it.amount || 0) - Number(it.discount || 0)), 0)
                        ...
                        className="w-full bg-white border border-slate-300 rounded-xl px-2.5 py-1.5 outline-none text-xs font-mono font-bold"
                      />
                    </div>

                    <div>
                      <label className="block font-bold text-amber-600 text-[10px] mb-1">Discount (₹)</label>
                      <input
                        type="number"
                        value={item.discount}
                        onChange={(e) => {
                          const currentItems = [...(newStudentForm.payment_items || [])];
                          const idxToUpdate = currentItems.findIndex(it => it.id === item.id);
                          if (idxToUpdate !== -1) currentItems[idxToUpdate].discount = parseFloat(e.target.value) || 0;
                          setNewStudentForm({
                            ...newStudentForm,
                            payment_items: currentItems,
                            total_fee: currentItems.reduce((sum: number, it: any) => sum + Math.max(0, Number(it.amount || 0) - Number(it.discount || 0)), 0)
                        ...
                        className="w-full bg-white border border-slate-300 rounded-xl px-2.5 py-1.5 outline-none text-xs font-mono font-bold text-amber-700"
                      />
                    </div>
                    ...
                    <div className="flex justify-end">
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-rose-500/10 text-rose-600 hover:bg-rose-600 hover:text-white rounded-xl text-[10px] font-bold transition w-full"
                      >
                        Remove
                      </button>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2 border-t">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1 font-mono">Net Due Total (₹)</label>
                  <div className="w-full flex items-center bg-slate-100 border border-slate-300 rounded-xl px-3 py-2 font-mono font-extrabold text-slate-800 text-sm">
                    ₹{(newStudentForm as any).total_fee || 0}
                  </div>
                </div>

                <div>
                  <label className="block font-bold text-slate-700 mb-1">Fee Collected / Paid (₹)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block font-bold text-rose-700 mb-1">Fee Pending / Due (₹) — auto</label>
                  <div className="w-full flex items-center bg-rose-50 border border-rose-200 rounded-xl px-3 py-2 font-mono font-extrabold text-rose-800 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Plan Validity End Date</label>
                  <input 
                    type="date" 
