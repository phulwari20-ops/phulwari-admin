...
html_template = """    let allFormsHtml = ''
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Bulk Parent Registration Forms</title>
          <style>
            @media print {
...
bulk_start = content.find("  const handleExportBulkRegistrationForms = () => {")
bulk_end = content.find("  // Print Registration PDF Form", bulk_start)
content = content[:bulk_start] + html_template + "\n" + content[bulk_end:]

single_start = content.find("  const handlePrintRegistrationForm = (st: any) => {")
